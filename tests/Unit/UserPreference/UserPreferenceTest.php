...
//    }
    ...
    /**
     * Test creating a new user preference by authentication.
     *
     * @return void
     */
    public function test_create_by_auth_user_preference()
    {
        $user = User::factory()->create();
        $token = $user->createToken('TestToken')->plainTextToken;
        $userPreference = UserPreference::factory()->make()->attributesToArray();
        ...
        $userPreference['preferred_authors'] = json_decode($userPreference['preferred_authors']);
        $response = $this->withHeaders([
            'Authorization' => 'Bearer ' . $token])->put('/api/user-preference/auth', $userPreference);

        $response->assertStatus(201);
    }

    /**
     * Test updating a new user preference by authentication.
     *
     * @return void
     */
    public function test_update_by_auth_user_preference()
    {
        $user = User::factory()->create();
        $token = $user->createToken('TestToken')->plainTextToken;
...
//    }
    ...
    public function test_get_user_preferences_by_auth()
    {
        $user = User::factory()->create();
        $token = $user->createToken('TestToken')->plainTextToken;

        $response = $this->withHeaders([
            'Authorization' => 'Bearer ' . $token])->get("/api/user-preference/auth");
